...
with model:
    ...
    actions = spa.Actions(
        'dot(rule, OBJ1*S+BELOW*V+OBJ2*O)*{strength} -dot(rule, (ABOVE+LEFT+RIGHT)*V) +'
        '(dot(objs, OBJ1*Y) - dot(objs, OBJ2*Y))+{separation} --> '
                 'status=BAD, '
                 'objs=-{speed}*Y*OBJ1 + {speed}*Y*OBJ2'.format(**locals()),
        'dot(rule, OBJ1*S+BELOW*V+OBJ2*O)*{strength} - '
        '(dot(objs, OBJ1*Y) - dot(objs, OBJ2*Y))-{separation} --> '
                  'status=GOOD'.format(**locals()),

        'dot(rule, OBJ1*S+ABOVE*V+OBJ2*O)*{strength} -dot(rule, (BELOW+LEFT+RIGHT)*V) +'
        '(dot(objs, OBJ2*Y) - dot(objs, OBJ1*Y))+{separation} --> '
                 'status=BAD, '
                 'objs=-{speed}*Y*OBJ2 + {speed}*Y*OBJ1'.format(**locals()),
        'dot(rule, OBJ1*S+ABOVE*V+OBJ2*O)*{strength} - '
        '(dot(objs, OBJ2*Y) - dot(objs, OBJ1*Y))-{separation} --> '
                  'status=GOOD'.format(**locals()),
